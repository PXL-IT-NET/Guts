...
import {
  IAssignmentModel,
  ITestModel,
} from "src/app/viewmodels/assignment.model";
...
export class AssignmentSettingsComponent {
  ...
  public deleteAssignment(assignment: IAssignmentModel) {
    let confirmMessage =
      ...
      "'? All tests and test results will also be deleted.";
        ...
        .subscribe((result: PostResult) => {
          ...
          if (result.success) {
            this.toastr.info(
              ...
              this.capitalizeFirstLetter(this.assignmentKind) + " deleted",
            );
            this.assignmentChanged.emit(this.assignment);
          } else {
            this.toastr.error(
              result.message || "unknown error",
              "Could not delete " + this.assignmentKind,
            );
          }
  ...
  public deleteTest(assignment: IAssignmentModel, test: ITestModel) {
    let confirmMessage =
      "Are you sure you want to remove the test '" +
      test.testName +
      "' from the " +
      this.assignmentKind +
      " '" +
      assignment.description +
      "'? The linked test results will also be deleted.";
    if (confirm(confirmMessage)) {
      this.loading = true;
      this.testService.deleteTest(test.id).subscribe((result: PostResult) => {
        this.loading = false;
        if (result.success) {
          this.toastr.info(
            "The test '" + test.testName + "' is succesfully deleted",
            "Test deleted",
          );
          this.assignmentChanged.emit(this.assignment);
        } else {
