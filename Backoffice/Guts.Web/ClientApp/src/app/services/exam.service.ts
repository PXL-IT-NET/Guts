...
export class ExamService {
  ...
  public getExamsOfCourse(
    courseId: number,
  ): Observable<GetResult<ExamModel[]>> {
    return this.http.get<IExamModel[]>("api/exams?courseId=" + courseId).pipe(
      map((examModels) =>
        GetResult.success(examModels.map((model) => new ExamModel(model))),
      ),
  ...
  public saveExam(model: IExamModel): Observable<CreateResult<ExamModel>> {
    return this.http.post<IExamModel>("api/exams", model).pipe(
  ...
  public saveExamPart(
    examId: number,
    examPart: ExamPartModel,
  ): Observable<CreateResult<ExamPartModel>> {
    ...
    return this.http
      .post<IExamPartModel>("api/exams/" + examId + "/parts", model)
      .pipe(
        ...
        catchError((errorResponse: HttpErrorResponse) => {
          return of(
            CreateResult.fromHttpErrorResponse<ExamPartModel>(errorResponse),
          );
        }),
      );
  }
  ...
  public deleteExamPart(
    ...
    return this.http
      .delete("api/exams/" + examId + "/parts/" + examPartId)
      .pipe(
  ...
  public getExamResultsDownloadUrl(examId: number): Observable<Result> {
    return this.http
      .get<any>("api/exams/" + examId + "/downloadscores", {
